...
int main(int argc, char const *argv[])
{
    ...
    if (listen(server_fd, 1) < 0) // 3 is the maximum size of queue - connections you haven't accepted
    {
        perror("listen");
        exit(EXIT_FAILURE);
    }
    ...
    while(1) {
        ...
        // printf("Connection Established from %s",inet_ntoa(address->sin_addr));
